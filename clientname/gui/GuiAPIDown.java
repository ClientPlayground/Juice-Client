...
public class GuiAPIDown extends GuiScreen {
	...
	public GuiAPIDown()
	{
		...
		message = new IChatComponent[] {
				...
				new ChatComponentText(""),
				new ChatComponentText("The Client is currently expecting server issues... "),
				new ChatComponentText(""),
				new ChatComponentText("Please Check back later or join our support discord to check the status of this issue"),
				new ChatComponentText(""),
				new ChatComponentText(HWID.get()).setChatStyle(
						new ChatStyle().setColor(EnumChatFormatting.GOLD)
						.setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ChatComponentText("Click here copy a Discord invite link").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GREEN))))
						.setChatClickEvent(new ClickEvent(ClickEvent.Action.COPY_CLIPBOARD, HWID.discord()))
						),
				new ChatComponentText(""),
		};
